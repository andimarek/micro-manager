...
function checkoutIntoTmp(
  repos: Repository[]
): Promise<{ [repoId: string]: string }> {
...
export function checkoutIntoWorkspace(
  workspace: string
...
export type PathByProjectId = {
  [projectId: string]: { projectPath: string; repoPath: string };
};

export function checkoutAllProjects(
  projects: Project[]
): Promise<PathByProjectId> {
  ...
  return checkoutIntoTmp(uniqueRepos).then(pathByRepoId => {
    ...
    const repoPathProjecTuples: RepoPathProject[] = flatMap(
      repoIds,
      (repoId): RepoPathProject[] => {
