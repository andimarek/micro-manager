...
class MessageFactory {
	...
	public static replyErr(req: string, err: any): string {
		...
		return `{"seq":"${req}","err":${marshalling.stringify(transformErrorForSerialization(err))}}`;
	}
}

type ValueCallback<T,TResult1> = (value: T) => TResult1 | PromiseLike<TResult1>;
type ErrorCallback<TResult2> = (reason: any) => TResult2 | PromiseLike<TResult2>;
...
function createRPC(serializeAndSend: (value: string) => void): IRPCFunc {
	...
	return function rpc(rpcId: string, method: string, args: any[]): Promise<any> {
...
export function createProxyProtocol(protocol: IMessagePassingProtocol): IRemoteCom {
	...
	function invokeHandler(rpcId: string, method: string, args: any[]): Promise<any> {
		try {
